...
def call_next(counter_id):
    """Counter calls next waiting ticket (Priority first, then FIFO)"""
...
def recall_ticket(counter_id):
    """Re-announce active ticket at counter with voice + chime"""
...
def start_serving(counter_id):
    """Mark active ticket as in-progress serving"""
...
def update_counter_status(counter_id):
    """Update counter status (available, break, offline) and officer name"""
    data = request.get_json(silent=True) or {}
...
def export_csv():
    ...
    writer.writerow([
        'Ticket #', 'Service Name', 'Category', 'Counter', 'Officer',
    ...
    for t in tickets:
        ...
        writer.writerow([
            t['ticketNumber'],
            t['serviceName'],
            t['priorityType'].upper() if t.get('isPriority') else 'REGULAR',
            t['counterName'] or '',
            t['officer'] or '',
            t['status'].upper(),
            t['waitSeconds'],
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=assessor_queue_report_{time.strftime("%Y%m%d_%H%M%S")}.csv'
